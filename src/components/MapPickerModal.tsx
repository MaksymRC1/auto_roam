...
const LoadingMap = () => {
  ...
  return <div className="w-full h-full flex items-center justify-center bg-black/20 backdrop-blur-sm animate-pulse text-white/50">{t('loadingMap')}</div>;
}
...
export function MapPickerModal({ 
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const cityName = geo?.city || t('pointOnMap');
      ...
      console.error(e);
      onConfirm(`${t('pointOnMap')} | ${selectedPos[0].toFixed(5)},${selectedPos[1].toFixed(5)}`);
    } finally {
      setIsLoading(false);
      onOpenChange(false);
      setSelectedPos(null);
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-2xl h-[80vh] flex flex-col p-4 w-[95vw] max-w-none sm:w-full border-white/10" style={{ background: "rgba(0, 0, 0, 0.45)", backdropFilter: "blur(16px)" }}>
        <DialogHeader>
          <DialogTitle>{t('selectPointTitle')}</DialogTitle>
        </DialogHeader>
        ...
        <DialogFooter className="mt-4 flex sm:justify-between items-center gap-4">
          <p className="text-sm text-slate-500 hidden sm:block">
            {selectedPos ? `${t('selected')} ${selectedPos[0].toFixed(4)}, ${selectedPos[1].toFixed(4)}` : t('clickToSelectPoint')}
          </p>
          <div className="flex gap-2 w-full sm:w-auto">
            <Button variant="outline" className="flex-1 sm:flex-none" onClick={() => handleOpenChange(false)}>{tCommon('cancel')}</Button>
